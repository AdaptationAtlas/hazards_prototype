...
risk_x_exposure <- function(file,
  ...
  if (!file.exists(save_name) || overwrite == TRUE) {
...
boundaries_zonal <- lapply(seq_along(Geographies), FUN = function(i) {
  ...
  if (!file.exists(file_path) | overwrite_boundary_zones == TRUE) {
    zones <- Geographies[[i]]
    zone_rast <- rasterize(
      x = zones,
      y = base_rast,
...
for (tx in seq_along(timeframe_choices)) {
  ...
  if (run1) {
    ...
    p <- with_progress({
      ...
      prog <- progressr::progressor(along = seq_along(mod_x_type_x_sev))

      invisible(
        future.apply::future_lapply(seq_along(mod_x_type_x_sev), FUN = function(i) {
          ...
          if (!file.exists(save_file) | overwrite1) {
            ...
            result <- rbindlist(lapply(seq_along(boundaries_zonal), FUN = function(k) {
  ...
  if (run2) {
    ...
    files <- data.table(file = list.files(haz_mean_dir, ".tif$", full.names = TRUE))
    files[, c("model", "base_stat") := tstrsplit(basename(file)[1], "_", keep = c(2, 5), fixed = TRUE), by = file][, base_stat := gsub(".tif", "", base_stat)][, mod_x_stat := paste0(model, "_", base_stat)]
    ...
    p <- with_progress({
      # Define the progress bar
      prog <- progressr::progressor(along = seq_along(ms_options))

      invisible(
        future.apply::future_lapply(seq_along(ms_options), FUN = function(i) {
  ...
  if (run3) {
    ...
    haz_timeseries_files <- list.files(haz_timeseries_dir, ".tif", full.names = TRUE)
    haz_timeseries_files <- grep(paste(hazards, collapse = "|"), haz_timeseries_files, value = TRUE)
    ...
    haz_timeseries_files <- grep("ENSEMBLEmean", haz_timeseries_files, value = TRUE)
    ...
    layer_names <- unlist(lapply(seq_along(haz_timeseries_files), FUN = function(i) {
      file <- haz_timeseries_files[i]
      layers <- names(terra::rast(file))
      file <- gsub(".tif", "", tail(unlist(strsplit(file, "/")), 1), fixed = TRUE)
    ...
    haz_timeseries_tab <- rbindlist(lapply(seq_along(levels), FUN = function(i) {
      level <- levels[i]
      print(level)
      ...
      data[, variable := gsub("historical", "historic-NA-historic", variable[1]), by = variable][, variable := stringi::stri_replace_all_regex(variable[1], pattern = c("max_max", "min_min", "mean_mean"), replacement = c("max-max", "min-min", "mean-mean"), vectorise_all = FALSE), by = variable][, variable := gsub(".", "_", variable[1], fixed = TRUE), by = variable][, variable := gsub("_year", "-", variable[1], fixed = TRUE), by = variable]
      ...
      variable[is.na(hazard_stat), hazard := gsub("_", "-", hazard)][is.na(hazard_stat), hazard_stat := unlist(tstrsplit(hazard, "-", keep = 2))][, hazard := unlist(tstrsplit(hazard, "-", keep = 1))]
  ...
  if (run4.1 || run4.2) {
    ...
    if (run4.1) {
      ...
      if (ensemble_only4.1) {
        files <- grep("ENSEMBLE|historic", files, value = TRUE)
      }
    ...
    if (run4.2) {
      ...
      for (v in seq_along(to_do_list)) {
        ...
        attr_notes <- paste0(
          "A table of hazard occurence (a proportion 0-1 as the average of of a binary timeseries of 0/1 present/absent hazard) multiplied by exposure (",
          variable,
        ...
        files_int[, c("crop", "model", "severity", "hazard_vars", "type", "exposure_var", "exposure_unit") := tstrsplit(basename(file)[1], "_", keep = 1:7, fixed = TRUE), by = file][, exposure_unit := gsub(".tif", "", exposure_unit)]
        ...
        p <- suppressWarnings(with_progress({
          ...
          furrr::future_map(seq_along(group), function(i) {
            ...
            if (!file.exists(save_file) | overwrite) {
              ...
              result_long_adm012 <- rbind(result_long, rbind(agg_admin1, agg_admin0, fill = TRUE), fill = TRUE)
        ...
        if (do_ensemble_sd4.2) {
          ...
          en_files_mean <- list.files(folder, "ENSEMBLEmean.*parquet$", full.names = TRUE)

          for (g in seq_along(en_files_mean)) {
