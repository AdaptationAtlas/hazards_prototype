...
if (FALSE) {
  ...
  set_parallel_plan(n_cores = 10, use_multisession = TRUE)
  ...
  existing_files <- existing_files[
    !grepl(
      "PRCPTOT-PTOT|PTOT-New-PTOT|PTOT-Updated-PTOT|GSeason|THI/THI_MAX/|_AgERA5/|stats|daily|historical/HSM_NTx|AVAIL.tif|LongTermMean|/max_year|/mean_monthly|/mean_year|/median_monthly|/median_year",
      existing_files
    )
  ]
  ...
  existing_files <- existing_files[, variable := dirname(file_path)][, scenario := basename(dirname(variable[1])), by = variable][, var_folder := basename(variable)][, base_name := gsub(".tif", "", basename(file_path))]
  ...
  existing_files[, var_file := unlist(tstrsplit(base_name[1], "-", keep = 1)), by = base_name][, year := unlist(tstrsplit(base_name[1], "-", keep = 2)), by = base_name][!grepl("TAI", base_name), month := unlist(tstrsplit(base_name[1], "-", keep = 3)), by = base_name]
...
if (FALSE) {
  ...
  set_parallel_plan(n_cores = 16, use_multisession = FALSE)
  handlers("progress")
...
for (ii in seq_len(nrow(parameters))) {
  ...
  for (season in 1:n_seasons) {
    ...
    if (worker_n > 1) {
      ...
      set_parallel_plan(n_cores = worker_n, use_multisession = TRUE)
      future::plan()
      ...
      p <- with_progress({
        ...
        progress <- progressr::progressor(along = seq_len(nrow(folders_x_hazards)))

        future.apply::future_lapply(
          X = seq_len(nrow(folders_x_hazards)),
          FUN = function(i) {
    ...
    if (do_ensemble) {
      ...
      p <- progressr::with_progress({
        ...
        progress <- progressr::progressor(along = seq_len(nrow(scen_haz_time)))

        n <- future_lapply(seq_len(nrow(scen_haz_time)), FUN = function(i) {
          progress(sprintf(
