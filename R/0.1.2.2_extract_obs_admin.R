...
# R/0.1.2_get_chirps_chirts.R + R/0.1.2.1_calculate_obs_spei.R from pixel
...
bootstrap_minimal <- function() {
  ...
  if (!dir.exists(chirts_chirps_hist_dir)) {
    stop("Run R/0.1.2_get_chirps_chirts.R --full before computing admin extract.")
  }
...
if (mode == "--smoke") {
  ...
  cat(
    "Usage:\n",
    "  Rscript R/0.1.2.2_extract_obs_admin.R --smoke\n",
    "      adm0 only, all 9 variables, last 5 years of coverage.\n",
    "  Rscript R/0.1.2.2_extract_obs_admin.R --full\n",
...
# was built by R/0.1.2_get_chirps_chirts.R into the project repo's metadata/.
...
for (level in levels_run) {
  ...
  tbl <- tbl$ReplaceSchemaMetadata(list(
    ...
    source = "R/0.1.2.2_extract_obs_admin.R against Data/chirts_chirps_hist/{PTOT,TMAX,TMIN,TAVG,SPEI-*}/*.tif",
    ...
    build_script = "R/0.1.2.2_extract_obs_admin.R"
  ))
  ...
  jsonlite::write_json(list(
    ...
    build_time = format(Sys.time(), "%Y-%m-%dT%H:%M:%S%z"),
    parent_script = "R/0.1.2.2_extract_obs_admin.R"
