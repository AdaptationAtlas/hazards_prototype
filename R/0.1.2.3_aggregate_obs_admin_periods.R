...
# (obs_monthly_adm{0,1}.parquet from R/0.1.2.2_extract_obs_admin.R) into a
...
if (mode == "--smoke") {
  ...
  cat(
    "Usage:\n",
    "  Rscript R/0.1.2.3_aggregate_obs_admin_periods.R --smoke\n",
    "      adm0 only; verification checks; exit 0/1.\n",
    "  Rscript R/0.1.2.3_aggregate_obs_admin_periods.R --full\n",
    "      adm0 + adm1.\n",
    sep = ""
  )
  quit(status = 1)
...
for (lvl in levels_run) {
  ...
  if (!file.exists(in_path)) {
    stop(glue::glue("Missing input: {in_path}. Run R/0.1.2.2_extract_obs_admin.R first."))
  }
  ...
  tbl <- tbl$ReplaceSchemaMetadata(list(
    ...
    source = sprintf(
      "R/0.1.2.3_aggregate_obs_admin_periods.R from %s", basename(in_path)
    ),
    ...
    build_script = "R/0.1.2.3_aggregate_obs_admin_periods.R"
  ))
  ...
  jsonlite::write_json(list(
    ...
    build_time = format(Sys.time(), "%Y-%m-%dT%H:%M:%S%z"),
    parent_script = "R/0.1.2.3_aggregate_obs_admin_periods.R"
