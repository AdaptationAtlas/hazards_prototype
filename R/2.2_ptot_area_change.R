...
# list data files
files<-list.files(haz_time_risk_dir,full.names =T)
files<-grep("THI_max",files,value=T)
...
cat_thresholds<-haz_class[index_name=="THI_max" & 
                            ...
                            ][,code2:=paste(c(unlist(tstrsplit(crop,"_",keep=2)),tolower(description)),collapse = "_"),by=code]
...
data<-lapply(1:nrow(cat_thresholds),FUN=function(i){
  files_ss<-grep(cat_thresholds[i,code],files,value=T)
  data<-terra::rast(files_ss)
  names(data)<-paste0(gsub(".tif","",basename(files_ss)),"_",cat_thresholds[i,code2])
  if(cat_thresholds[i,grepl("tropical",crop)]){
    data<-data*cat_mask_tropical
  }else{
...
data_trop<-grep("tropical",cat_thresholds$crop)


data<-base_cellsize*data
