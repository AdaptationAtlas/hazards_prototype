...
haz_class <- rbind(
  haz_class[crop == "generic"],
  rbindlist(lapply(seq_along(livestock), FUN = function(i) {
...
ecocrop[, Temp_Abs_Min := as.numeric(Temp_Abs_Min)][, Temp_Abs_Max := as.numeric(Temp_Abs_Max)][, Rain_Abs_Min := as.numeric(Rain_Abs_Min)][, Rain_Abs_Max := as.numeric(Rain_Abs_Max)]
...
ec_haz <- rbindlist(lapply(seq_len(nrow(ms_codes)), FUN = function(i) {
  ...
  ec_haz <- rbindlist(lapply(seq_along(crops), FUN = function(j) {
    ...
    if (nrow(ecrop) > 0) {
      ...
      ptot_low <- data.table(
        ...
        threshold = c(
          ...
          unlist(ecrop$Rain_Abs_Min)
        )
      ) # Extreme

      # PTOT high
      ptot_high <- data.table(
        index_name = "PTOT",
        description = description,
        direction = ">",
        crop = crop_common,
      ...
      tavg_low <- data.table(
        ...
        threshold = c(
          ...
          unlist(ecrop$Temp_Abs_Min)
        )
      ) # Extreme

      # TAVG high
      tavg_high <- data.table(
        index_name = "TAVG",
        description = description,
        direction = ">",
        crop = crop_common,
        threshold = c(
          ...
          unlist(ecrop$Temp_Abs_Max)
        )
      ) # Extreme

      # NTxCrop - moderate  (>optimum)
      ntxcrop_m <- data.table(
        index_name = paste0("NTxM", ecrop$Temp_Opt_Max),
        description = description,
        direction = ">",
        crop = crop_common,
        threshold = c(
      ...
      # NTxCrop - severe
      ntxcrop_s <- data.table(
        index_name = paste0("NTxS", ceiling((unlist(ecrop$Temp_Opt_Max) + unlist(ecrop$Temp_Abs_Max)) / 2)),
        description = description,
        direction = ">",
        crop = crop_common,
        threshold = c(
      ...
      ntxcrop_e <- data.table(
        index_name = paste0("NTxE", ecrop$Temp_Abs_Max),
        description = description,
        direction = ">",
        crop = crop_common,
        threshold = c(
  ...
  ec_haz <- unique(ec_haz[grep("NTxS", index_name), index_name := paste0("NTxS", ceiling(mean(as.numeric(substr(index_name, 5, 6)))))][grep("NTxM", index_name), index_name := paste0("NTxM", ceiling(mean(as.numeric(substr(index_name, 5, 6)))))][grep("NTxE", index_name), index_name := paste0("NTxE", ceiling(mean(as.numeric(substr(index_name, 5, 6)))))])

  # Average threholds where multiple crops exist for a mapspam commodity
  ec_haz <- ec_haz[, list(threshold = mean(threshold, na.rm = TRUE)), by = list(index_name, description, direction, crop)]
...
haz_class[, direction2 := "G"][direction == "<", direction2 := "L"][, index_name2 := index_name]
...
haz_class <- merge(haz_class, unique(haz_meta[, c("variable.code", "function")]), by.x = "index_name", by.y = "variable.code", all.x = TRUE)
...
haz_class[, match_field := code2][grepl("PTOT|TAVG", code2), match_field := paste0(index_name2, "_", `function`), by = .I]

haz_class <- merge(haz_class, unique(haz_meta[, list(code2, type)]), by.y = "code2", by.x = "match_field", all.x = TRUE)
...
short_crops <- spam_meta[long_season == FALSE, Fullname]
...
animal_interactions <- data.table(
  heat_simple = c("THI_max", "THI_max"),
  wet_simple = c("NDWL0", "PTOT_G"),
  dry_simple = c("NDWS", "PTOT_L"),
  heat_fixed = c(FALSE, FALSE),
  wet_fixed = c(TRUE, FALSE),
  dry_fixed = c(TRUE, FALSE),
  type = "animal"
)

interaction_haz <- unique(c(
  unlist(crop_interactions[, .(heat_simple, wet_simple, dry_simple)]),
  unlist(animal_interactions[, .(heat_simple, wet_simple, dry_simple)])
))

interaction_haz_fixed <- unique(c(
  crop_interactions[heat_fixed == TRUE, heat_simple],
  crop_interactions[wet_fixed == TRUE, wet_simple],
  crop_interactions[dry_fixed == TRUE, dry_simple],
  animal_interactions[heat_fixed == TRUE, heat_simple],
  animal_interactions[wet_fixed == TRUE, wet_simple],
  animal_interactions[dry_fixed == TRUE, dry_simple]
))

interaction_haz_free <- unique(c(
  crop_interactions[heat_fixed == FALSE, heat_simple],
  crop_interactions[wet_fixed == FALSE, wet_simple],
  crop_interactions[dry_fixed == FALSE, dry_simple],
  animal_interactions[heat_fixed == FALSE, heat_simple],
  animal_interactions[wet_fixed == FALSE, wet_simple],
  animal_interactions[dry_fixed == FALSE, dry_simple]
))
...
# Create a unique list of all the 3-way combinations required for the crops and severity classes selected
combinations_a <- unique(rbindlist(lapply(seq_along(livestock_choices), FUN = function(i) {
  crop_focus <- livestock_choices[i]
  result <- rbindlist(lapply(seq_along(severity_classes$class), FUN = function(j) {
    severity_focus <- severity_classes$class[j]
...
combinations_ca <- rbind(combinations_c, combinations_a)[, combo_name := paste0(c(dry, heat, wet), collapse = "+"), by = list(dry, heat, wet, crop, severity_class)][, severity_class := tolower(severity_class)]

combinations_ca[, heat1 := stringi::stri_replace_all_regex(heat, pattern = haz_meta[, gsub("_", "-", code)], replacement = haz_meta[, paste0(code, "-")], vectorise_all = FALSE)][, heat1 := unlist(tstrsplit(heat1, "-", keep = 1))]
combinations_ca[, dry1 := stringi::stri_replace_all_regex(dry, pattern = haz_meta[, gsub("_", "-", code)], replacement = haz_meta[, paste0(code, "-")], vectorise_all = FALSE)][, dry1 := unlist(tstrsplit(dry1, "-", keep = 1))]
combinations_ca[, wet1 := stringi::stri_replace_all_regex(wet, pattern = haz_meta[, gsub("_", "-", code)], replacement = haz_meta[, paste0(code, "-")], vectorise_all = FALSE)][, wet1 := unlist(tstrsplit(wet1, "-", keep = 1))]

combinations_ca[, combo_name1 := paste0(c(dry1[1], heat1[1], wet1[1]), collapse = "+"), by = list(dry1, heat1, wet1)]
combinations_ca[, combo_name_simple2 := paste0(c(
  gsub("_", "-", dry_simple[1]),
  gsub("_", "-", heat_simple[1]),
  gsub("_", "-", wet_simple[1])
), collapse = "+"), by = list(dry_simple, heat_simple, wet_simple)]
...
combinations_ca_ss <- combinations_c[crop %in% short_crops][, combo_name := paste0(c(dry, heat, wet), collapse = "+"), by = list(dry, heat, wet, crop, severity_class)][, severity_class := tolower(severity_class)]

combinations_ca_ss[, heat1 := stringi::stri_replace_all_regex(heat, pattern = haz_meta[, gsub("_", "-", code)], replacement = haz_meta[, paste0(code, "-")], vectorise_all = FALSE)][, heat1 := unlist(tstrsplit(heat1, "-", keep = 1))]
combinations_ca_ss[, dry1 := stringi::stri_replace_all_regex(dry, pattern = haz_meta[, gsub("_", "-", code)], replacement = haz_meta[, paste0(code, "-")], vectorise_all = FALSE)][, dry1 := unlist(tstrsplit(dry1, "-", keep = 1))]
combinations_ca_ss[, wet1 := stringi::stri_replace_all_regex(wet, pattern = haz_meta[, gsub("_", "-", code)], replacement = haz_meta[, paste0(code, "-")], vectorise_all = FALSE)][, wet1 := unlist(tstrsplit(wet1, "-", keep = 1))]

combinations_ca_ss[, combo_name1 := paste0(c(dry1[1], heat1[1], wet1[1]), collapse = "+"), by = list(dry1, heat1, wet1)]
combinations_ca_ss[, combo_name_simple2 := paste0(c(
  gsub("_", "-", dry_simple[1]),
  gsub("_", "-", heat_simple[1]),
  gsub("_", "-", wet_simple[1])
), collapse = "+"), by = list(dry_simple, heat_simple, wet_simple)]
...
Thresholds_U[, direction2 := direction][, direction2 := gsub("<", "L", direction2)][, direction2 := gsub(">", "G", direction2)][, code := paste0(direction2, threshold), by = .I][, index_name2 := index_name][index_name2 %in% c("PTOT", "TAVG"), index_name2 := paste0(index_name2, "_", direction2)]

# Subset to interaction hazards
Thresholds_U <- Thresholds_U[grepl(paste(if (any(grepl("NTx", interaction_haz))) {
  c("NTx", interaction_haz)
} else {
  interaction_haz
}, collapse = "|"), index_name2)]

# Do the same for annual crops only
Thresholds_U_ss <- unique(haz_class[crop %in% short_crops & description != "No significant stress", list(index_name, code2, direction, threshold)][, index_name := gsub("NTxM|NTxS|NTxE", "NTx", index_name)])

Thresholds_U_ss[, direction2 := direction][, direction2 := gsub("<", "L", direction2)][, direction2 := gsub(">", "G", direction2)][, code := paste0(direction2, threshold), by = .I][, index_name2 := index_name][index_name2 %in% c("PTOT", "TAVG"), index_name2 := paste0(index_name2, "_", direction2)]

# Subset to interaction hazards
Thresholds_U_ss <- Thresholds_U_ss[grepl(paste(if (any(grepl("NTx", interaction_haz))) {
  c("NTx", interaction_haz)
} else {
  interaction_haz
}, collapse = "|"), index_name2)]
...
for (tx in seq_along(timeframes)) {
  timeframe <- timeframes[tx]

  start_time <- Sys.time()
  cat(
    "Processing", timeframe, tx, "/", length(timeframes),
  ...
  if (run1) {
    ...
    p <- with_progress({
      ...
      prog <- progressr::progressor(along = seq_len(nrow(thresholds)))

      future.apply::future_lapply(seq_len(nrow(thresholds)), FUN = function(i) {
        #  lapply(1:nrow(thresholds),FUN=function(i){
        index_name <- thresholds[[i, "code2"]]
        files_ss <- grep(index_name, files, value = TRUE)
        ...
        for (j in seq_along(files_ss)) {
          #   cat(i,"-",j,"\n")

          file_name <- gsub(".tif", paste0("-", thresholds[[i, "code"]], ".tif"), file.path(haz_time_class_dir, basename(files_ss[j])), fixed = TRUE)
          ...
          if ((!file.exists(file_name)) | overwrite1) {
            ...
            terra::writeRaster(data_class, filename = file_name, overwrite = TRUE, filetype = "COG", gdal = c("OVERVIEWS" = "NONE"))
            rm(data, data_class)
            gc()
          }
        }
  ...
  if (run2) {
    ...
    if (run2_main) {
      ...
      p <- with_progress({
        ...
        invisible(future.apply::future_lapply(seq_along(files), FUN = function(i) {
          ...
          if ((!file.exists(file)) | overwrite2) {
            ...
            terra::writeRaster(data, filename = file, overwrite = TRUE, filetype = "COG", gdal = c("OVERVIEWS" = "NONE"))
            rm(data)
            gc()
          }
        }))
    ...
    if (do_ensemble2) {
      ...
      p <- with_progress({
        # Define the progress bar
        progress <- progressr::progressor(along = seq_len(nrow(haz_scen_mod)))

        invisible(future.apply::future_lapply(seq_len(nrow(haz_scen_mod)), FUN = function(i) {
          ...
          if (!file.exists(save_file_mean) | overwrite2) {
            ...
            terra::writeRaster(ensemble_mean, filename = save_file_mean, overwrite = TRUE, filetype = "COG", gdal = c("OVERVIEWS" = "NONE"))
            terra::writeRaster(ensemble_sd, filename = save_file_sd, overwrite = TRUE, filetype = "COG", gdal = c("OVERVIEWS" = "NONE"))
    ...
    if (check2) {
      ...
      result <- check_tif_integrity(
        dir_path = haz_time_risk_dir,
        recursive = FALSE,
  ...
  if (run3) {
    ...
    p <- with_progress({
      # Define the progress bar
      progress <- progressr::progressor(along = seq_along(crops))

      invisible(future.apply::future_lapply(seq_along(crops), FUN = function(i) {
        # Display progress
        ...
        for (j in seq_len(nrow(severity_classes))) {
          ...
          for (k in seq_along(models)) {
            ...
            if (!file.exists(save_name) | overwrite3 == TRUE) {
              haz_class_df_subset <- haz_class_df[haz_class_df$crop == crop_focus &
                haz_class_df$description == sev_choice &
                haz_class_df$index_name2 %in% if (crop_focus == "generic-crop") {
                  interaction_haz_fixed
                } else {
                  interaction_haz_free
                }, c("type", "filename", "index_name2")]
              ...
              if (nrow(files) == 0) {
                stop(
                  "Crop:", i, "/", length(crops), crop_focus, "| severity:", j, "/", length(severity_classes$class), sev_choice, "| model:", k, "/", length(models), model_k,
                  " - filename is empty               \r"
                )
              }
              ...
              files <- merge(files, haz_class_df_subset, by = "hazard2", all.x = TRUE, sort = FALSE)
  ...
  if (run4) {
    ...
    p <- with_progress({
      ...
      invisible(future.apply::future_lapply(seq_along(files), FUN = function(i) {
        ...
        if ((!file.exists(file_mean)) | overwrite4) {
          data <- terra::rast(files[i])
          data_mean <- terra::mean(data, na.rm = TRUE)
          data_sd <- terra::app(data, fun = sd, na.rm = TRUE)
          ...
          terra::writeRaster(data_mean, filename = file_mean, overwrite = TRUE, filetype = "COG", gdal = c("COMPRESS=LZW", of = "COG", "OVERVIEWS" = "NONE"))
          terra::writeRaster(data_sd, filename = file_sd, overwrite = TRUE, filetype = "COG", gdal = c("COMPRESS=LZW", of = "COG", "OVERVIEWS" = "NONE"))
    ...
    if (do_ensemble4) {
      ...
      file_tab[, stat := gsub("[.]tif", "", stat)][, file := files]
      ...
      p <- with_progress({
        ...
        invisible(future.apply::future_lapply(seq_len(nrow(haz_scen_mod)), FUN = function(i) {
          ...
          if (length(ensemble_files) != length(model_options)) {
            stop(
              "4.1) Error - number of GCMS (", length(ensemble_files), ") !=", "number of models (", length(model_options),
    ...
    if (FALSE) {
      ...
      if (!file.exists(change_file) || overwrite == TRUE) {
        change <- pblapply(seq_along(files_hist), FUN = function(i) {
          # Display progress
  ...
  haz_class_file_tab[, hazard := gsub("[.]tif", "", hazard)][, file := haz_class_files][, hazard2 := unlist(tstrsplit(hazard, "-", keep = 1))]
  ...
  if (run5.2) {
    ...
    p <- with_progress({
      ...
      invisible(.lapply_fn(seq_len(nrow(combinations_choice)), FUN = function(i) {
        ...
        combo_binary <- data.table(combo_name = combo_names, value = 0)[grep(names(combos)[1], combo_name), value := 1][grep(names(combos)[2], combo_name), value := value + 10][grep(names(combos)[3], combo_name), value := value + 100]

        if (do5.2_main) {
          for (l in seq_len(nrow(scenarios_x_models))) {
            ...
            # Display progress
            cat(
              "Combination:", i, "/", nrow(combinations_choice),
            ...
            if (!file.exists(save_file) | overwrite5.2 == TRUE) {
        ...
        for (l in seq_len(nrow(scenarios))) {
          cat(
            "Calculating Ensemble:", i, "/", nrow(combinations_choice),
  ...
  if (run5.3) {
    ...
    haz_int_file_tab[, hazard := gsub("[.]tif", "", hazard)][, file := haz_int_files]
    ...
    p <- with_progress({
      # Define the progress bar
      progress <- progressr::progressor(along = seq_along(combinations_crops_choice))

      future.apply::future_lapply(seq_along(combinations_crops_choice), FUN = function(i) {
        ...
        for (j in seq_len(nrow(severity_classes))) {
          ...
          for (m in seq_along(model_options)) {
            ...
            invisible(lapply(seq_len(nrow(subset)), FUN = function(k) {
              combo_name <- subset$combo_name[k]
              combo_simple2 <- subset$combo_name_simple2[k]

              # Display progress
              cat(
                "crop_choice (i):", i, "/", length(combinations_crops),
              ...
              if (!file.exists(save_file) | overwrite5.3 == TRUE) {
                ...
                if (model_choice == "historic") {
                  if (length(files) != 1) {
                    stop(
                      "5.3) there should be one interaction stack for historic timeframe",
                  ...
                  if (length(files) < length(timeframe_options)) {
                    stop(
                      "5.3) there should be ", length(timeframe_options), " interaction stacks",
                      " | i = ", i, "/", crop_choice, " | j  = ", j, "/", sev_choice, " | m = ", m, "/", model_choice,
                      ", but n = ", length(files)
                    )
                  }
                ...
                terra::writeRaster(data, filename = save_file, overwrite = TRUE, filetype = "COG", gdal = c("OVERVIEWS" = "NONE"))
              }
            }))
          }
        }
  ...
  end_time <- Sys.time()
  cat(
    "Processing of", timeframe, tx, "/", length(timeframes),
...
if (FALSE) {
  ...
  if (upload2) {
    ...
    for (tx in seq_along(timeframes)) {
      ...
      # Local files
      local_files <- list.files(haz_time_risk_dir, "[.]tif$", full.names = TRUE)
  ...
  if (upload3) {
    ...
    for (tx in seq_along(timeframes)) {
      ...
      # Local files
      local_files <- list.files(haz_risk_dir, "[.]tif$", full.names = TRUE)
