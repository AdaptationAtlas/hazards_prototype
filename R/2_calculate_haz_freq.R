...
# 0) Set-up workspace ####
  ...
  ## 0.2) Set up workspace #####
    ...
    if(climdat_source=="atlas_delta"){
      ...
      Scenarios<-rbind(data.table(Scenario="historic",Time="1995_2014"),data.table(expand.grid(Scenario=Scenarios,Time=Times)))  
    }
  ...
  ## 0.3) Set flow controls and overwrite parameters ####
    ...
    # Interaction Tifs
    run5.2<-T
    do5.2_main<-T # Set to F if you only want to run the ensembling step
    check5.2<-T
    round5.2<-3
    overwrite5.2<-F
    do_ensemble5.2<-T
...
for(tx in 1:length(timeframes)){
  ...
  if(run1){
        ...
        for(j in 1:length(files_ss)){
         # cat("i =",i,"| j = ",j,"\n")
          ...
          if(climdat_source=="nexgddp"){
            ...
            file_name<-gsub("1995_2014_","1995-2014_",file_name)
          }
    ...
    # 1.1) Check results ######
    if(check1==T){
      cat(timeframe,"1) Classify time series climate variables based on hazard thresholds - Checking File Integrity\n")
    ...
    cat(timeframe,"1) Classify time series climate variables based on hazard thresholds - Check Completed\n")
    ...
    if(do_ensemble2){
      ...
      p<-with_progress({
        ...
        invisible(future.apply::future_lapply(1:nrow(haz_scen_mod),FUN=function(i){
          ...
          if(!file.exists(save_file_mean)|overwrite_ens2){
      ...
      invisible(future.apply::future_lapply(1:length(crops),FUN=function(i){
        ...
        for(j in 1:nrow(severity_classes)){
          ...
          for(k in 1:length(models)){
            ...
            # Display progress
            cat("Crop:",i,"/",length(crops),crop_focus,"| severity:",j,"/",length(severity_classes$class),sev_choice,"| model:",k,"/",length(models),model_k,"               \n")
            ...
            if(!file.exists(save_name)|overwrite3==T){
              ...
              if(nrow(files)==0){
                stop("Crop:i =",i,"/",length(crops),crop_focus,"| severity:j =",j,"/",length(severity_classes$class),sev_choice,"| model: k =",k,"/",length(models),model_k,
                     " - filename is empty               \r")
              }
  ...
  if(run4){
    ...
    files_new<-gsub("21_20","21-20",basename(files))
    files_new<-gsub("41_20","41-20",basename(files_new))
    files_new<-gsub("61_20","61-20",basename(files_new))
    files_new<-gsub("81_21","81-21",basename(files_new))
    files_new<-gsub("95_20","95-20",basename(files_new))
    ...
    if(do_ensemble4){
      ...
      # Use this code to identify any missing gcms
      #file_check<-dcast(file_tab,hazard+scenario+timeframe~model,fun=length)
      ...
      p<-with_progress({
        ...
        invisible(future.apply::future_lapply(1:nrow(haz_scen_mod),FUN=function(i){
          ...
          if(!file.exists(save_file_mean)|overwrite_ens4){
  ...
  # 5) Interactions ####
  ## 5.1) List classified raster stacks ####
    ...
    p<-with_progress({
      ...
      invisible(future.apply::future_lapply(1:nrow(combinations_choice), FUN=function(i){
        ...
        if(do5.2_main){
          for(l in 1:nrow(scenarios_x_models)){
            ...
            if(!file.exists(save_file)|overwrite5.2==T){
              ...
              files<-lapply(1:length(combos),function(ii){
                combo_i<-gsub("-","|",combos[ii])
                
                files[
                  Reduce(`&`, lapply(unlist(strsplit(combos[ii],"-")), function(p) grepl(p, files$hazard2))),"file"
                ]
        ...
        for(l in 1:nrow(scenarios)){
          ...
          # Ensemble models
          if(((climdat_source != "atlas_delta") | (scenario_choice != "historic")) & do_ensemble5.2){
