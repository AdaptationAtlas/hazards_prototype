...
glw2atlas <- list(
  poultry = grep("poultry", names(lps2fao), value = TRUE),
  sheep = grep("sheep", names(lps2fao), value = TRUE),
  pigs = grep("pig", names(lps2fao), value = TRUE),
  goats = grep("goat", names(lps2fao), value = TRUE),
  cattle = grep("cattle", names(lps2fao), value = TRUE)
)
...
prod_value_usd <- unique(prepare_fao_data(
  file = vop_file,
  lps2fao_ind,
  elements = element,
...
prod_value_usd_world <- prod_value_usd_world[Area == "World" & Element == element & Item %in% lps2fao_ind, c("Item", paste0("Y", target_year)), with = FALSE]
prod_value_usd_world <- merge(prod_value_usd_world, data.table(Item = lps2fao_ind, atlas_name = names(lps2fao_ind)), all.x = TRUE)
...
prod_value_i <- unique(prepare_fao_data(
  file = vop_file,
  lps2fao_ind,
  elements = element,
  remove_countries = remove_countries,
...
prod_value_i_world <- prod_value_i_world[Area == "World" & Element == element & Item %in% lps2fao_ind, c("Item", paste0("Y", target_year)), with = FALSE]
prod_value_i_world <- merge(prod_value_i_world, data.table(Item = lps2fao_ind, atlas_name = names(lps2fao_ind)), all.x = TRUE)
...
prod_price <- unique(prepare_fao_data(
  file = econ_file,
  lps2fao,
  elements = element,
  remove_countries = remove_countries,
...
if (FALSE) {
  ...
  ppp_xrat <- merge(ppp, xrat, all.x = TRUE)
...
nominal_usd <- lapply(seq_along(year_sets), function(i) {
  ...
  prod_price_global_recent <- prod_price_global[year %in% year_sets[[i]], .(price_usd_global = median(price_usd, na.rm = TRUE)), .(atlas_name)]
  ...
  prod_merge_recent[, price_usd_final := price_usd][is.na(price_usd_final), price_usd_final := price_median][is.na(price_usd_final), price_usd_final := price_usd_neighbors][
  ...
  ][atlas_name %in% c("goat_milk", "sheep_milk"), price_usd_final := price_usd_global][is.na(price_usd_final), price_usd_final := price_usd_continent][is.na(price_usd_final), price_usd_final := price_usd_global][, vop_usd_nominal := production_t * price_usd_global][, year := names(year_sets)[i]]
...
## 4.2) iusd2015 ####
intd_2015 <- lapply(seq_along(year_sets), function(i) {
  prod_merge_recent <- prod_merge[
    year %in% year_sets[[i]], .(vop_intd15 = median(value_intd15, na.rm = TRUE) * 1000),
    .(atlas_name, iso3)
  ]

  prod_merge_recent <- merge(prod_merge_recent, nominal_usd[[i]][, .(atlas_name, iso3, vop_usd_nominal)], all.x = TRUE)
...
for (i in seq_along(vop_list)) {
  ...
  final_vop_merge <- merge(final_vop, glw_admin0, all.x = TRUE, by = c("iso3", "glw3_name"))
  ...
  final_vop_vect <- merge(final_vop_vect, final_vop_cast, all.x = TRUE)
...
if (FALSE) {
  ...
  qa_rasterizer <- function(data, base_rast, geoboundaries, glw2atlas) {
    ...
    data_ex <- terra::extract(data, geoboundaries, fun = "sum", na.rm = TRUE)
    data_ex$ID <- NULL
    ...
    data_ex_vect <- merge(data_ex_vect, data_ex, all.x = TRUE, by = "iso3")
    ...
    list(table = data_ex, vect = data_ex_vect, rast = data_ex_rast)
  }
  ...
  fao_vop_cusd15 <- merge(fao_vop_cusd15, glw2atlas, all.x = TRUE, by = "atlas_name")
  fao_vop_cusd15 <- fao_vop_cusd15[, list(value = sum(Y2015, na.rm = TRUE), N = sum(is.na(Y2015))), by = list(glw3_name, iso3)]
  fao_vop_cusd15 <- fao_vop_cusd15[N == 0]

  fao_vop_cusd15 <- dcast(fao_vop_cusd15, iso3 ~ glw3_name)
  fao_vop_cusd15_vect <- geoboundaries
  fao_vop_cusd15_vect <- merge(fao_vop_cusd15_vect, fao_vop_cusd15, all.x = TRUE, by = "iso3")
  ...
  fao_vop_usd15 <- prepare_fao_data(
    file = vop_file,
    lps2fao_ind,
    elements = element,
    remove_countries = remove_countries,
  ...
  fao_vop_usd15 <- merge(fao_vop_usd15, glw2atlas, all.x = TRUE, by = "atlas_name")
  fao_vop_usd15 <- fao_vop_usd15[, list(value = sum(Y2015, na.rm = TRUE), N = sum(is.na(Y2015))), by = list(glw3_name, iso3)]
  fao_vop_usd15 <- fao_vop_usd15[N == 0]

  fao_vop_usd15 <- dcast(fao_vop_usd15, iso3 ~ glw3_name)
  fao_vop_usd15_vect <- geoboundaries
  fao_vop_usd15_vect <- merge(fao_vop_usd15_vect, fao_vop_usd15, all.x = TRUE, by = "iso3")
  ...
  fao_vop_id15 <- merge(fao_vop_id15, glw2atlas, all.x = TRUE, by = "atlas_name")
  fao_vop_id15 <- fao_vop_id15[, list(value = sum(Y2015, na.rm = TRUE), N = sum(is.na(Y2015))), by = list(glw3_name, iso3)]
  fao_vop_id15 <- fao_vop_id15[N == 0]

  fao_vop_id15 <- dcast(fao_vop_id15, iso3 ~ glw3_name)
  fao_vop_id15_vect <- geoboundaries
  fao_vop_id15_vect <- merge(fao_vop_id15_vect, fao_vop_id15, all.x = TRUE, by = "iso3")
