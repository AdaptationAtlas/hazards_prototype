...
# https://github.com/EiA2030/climate_prioritization/blob/main/R/0_create_global_crop_vop.R
...
# b) Set up workspace ####
spam_dir<-file.path(dirname(mapspam_dir),"spam2020V1r0_ifpri")
...
s3_files<-grep("cgiar",s3_files,value=T)
...
file<-grep("shp$",local_files,value=T)
...
target_year<-c(2013,2014,2015,2016,2017)
  ...
  ## 3.2) Load data #####
    ...
    prod_value_i[,value:=Y2015
                 ][is.na(value),value:=rowMeans(.SD,na.rm=T),.SDcols = val_cols[2:4]
