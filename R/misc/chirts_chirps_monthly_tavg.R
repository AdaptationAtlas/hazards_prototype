...
save_dir <- "~/common_data/hazards_prototype/Data/chirts_chirps_hist"

# 2) Set-up workspace ####
# 2.1) Set parameters #####
round_by <- 2
years_buff <- c(1993, 2016)
year_range <- (1995 - 2):(2014 + 2)
levels <- c(admin0 = "adm0", admin1 = "adm1") #,admin2="adm2")

# 2.2) List files #####
files_tmax <- data.table(
  file_path = list.files(
    file.path(data_dir_chirts, "Tmax", years_buff[1]:years_buff[2]),
    full.names = T
  ),
  variable = "tmax"
)
files_tmin <- data.table(
  file_path = list.files(
    file.path(data_dir_chirts, "Tmin", years_buff[1]:years_buff[2]),
    full.names = T
  ),
  variable = "tmin"
)
files_prec <- data.table(
  file_path = list.files(data_dir_chirps, "tif$", full.names = T),
  variable = "prec"
)
...
file_paths <- rbind(
  file_paths[,
    year := as.integer(unlist(tstrsplit(basename(file_path), "[.]", keep = 2)))
  ][,
  ...
  files_prec[,
    year := as.integer(unlist(tstrsplit(basename(file_path), "[.]", keep = 3)))
  ][,
...
times <- rbind(
  copy(times)[,
    file_name := file.path(
      monthly_dir_chirts,
      paste0("tavg-", year, "-", month, ".tif")
    )
  ][, var := "tavg"],
  copy(times)[,
    file_name := file.path(
      monthly_dir_chirps,
      paste0("prec-", year, "-", month, ".tif")
    )
  ][, var := "prec"]
)
...
process_time_step <- function(i, times, file_paths, base_rast_path) {
  ...
  if (!file.exists(save_file)) {
    ...
    if (var == "tavg") {
      files_max <- file_paths[
        year == times[i]$year & month == times[i]$month & variable == "tmax",
        file_path
      ]
      files_min <- file_paths[
        year == times[i]$year & month == times[i]$month & variable == "tmin",
        file_path
      ]
      ...
      files <- file_paths[
        year == times[i]$year & month == times[i]$month & variable == "prec",
        file_path
      ]
      rast_var <- terra::rast(files)
      rast_var <- terra::crop(rast_var, base_rast)
      rast_var <- sum(rast_var)
      rast_var <- terra::classify(
        rast_var,
        data.frame(from = -99999999, to = -0.0000001, becomes = NA)
      )
    }
...
# 4) Extract by admin ####
overwrite <- T # overwrite folder level extractions
save_file <- file.path(save_dir, "chirts_chirps_monthly_1993-2016.parquet")

if (!file.exists(save_file) | overwrite) {
  files <- c(
    list.files(monthly_dir_chirts, "tif$", full.names = T),
    list.files(monthly_dir_chirps, "tif$", full.names = T)
  )
  rast_stack <- terra::rast(files)
  names(rast_stack) <- gsub(".tif", "", basename(files))

  # data_ex <- admin_extract(data=rast_stack, boundaries_zonal, boundaries_index, FUN = "mean", max_cells_in_memory = 1*10^9)
  result <- rbindlist(purrr::map2(
    boundaries_zonal,
    boundaries_index,
    function(zonal_rast, idx) {
      zonal_r <- terra::rast(zonal_rast)
      dat <- zonal(rast_stack, zonal_r, fun = "mean", na.rm = TRUE)
      dat <- as.data.table(merge(
        dat,
        idx,
        by = "zone_id",
        all.x = TRUE,
  ...
  data_ex <- data.table::melt(data.table(result), id.vars = admin)
  ...
  data_ex <- data_ex[,
    ...
    variable,
    value
  )][, value := round(value, 1)][order(admin0_name, admin1_name, year, month)]
...
names(three_month_periods) <- sapply(1:12, function(start) {
  ...
  if (end <= 12) {
    ...
    return(paste(
      c(month_abbr[start:12], month_abbr[1:(end - 12)]),
      collapse = ""
    ))
  }
})
...
data_ex_season <- rbindlist(pblapply(
  1:length(three_month_periods),
  FUN = function(i) {
    ...
    data <- data[!is.na(seq) | seq == 0][,
      year := year[1],
      by = list(admin0_name, admin1_name, variable, seq)
    ][, seq := NULL]

    # Summarize the data for the variable
    data <- data[,
      .(
        value = if (variable == "tavg") {
...
# Add historical mean
data_ex_season[,
  baseline_mean := round(mean(value), round_by),
  by = list(
    admin0_name,
    admin1_name,
    gaul0_code,
...
unique(data_ex_season[
  value == -Inf | is.infinite(value) | is.na(value) | is.null(value),
  list(admin0_name, admin1_name, variable)
])
...
data_ex_season_stats <- data_ex_season[,
  .(
    value_s5 = round(mean(value[year %in% 1993:1997]), round_by),
...
data_ex_season_stats_simple <- data_ex_season_stats[, list(
  admin0_name,
  admin1_name,
  gaul0_code,
  gaul1_code,
...
save_file <- file.path(
  save_dir,
  "chirts_chirps_seasonal-stats_1995-2014.parquet"
)
arrow::write_parquet(data_ex_season_stats, sink = save_file)

save_file <- file.path(
  save_dir,
  "chirts_chirps_seasonal-stats_1995-2014_simple.parquet"
)
...
files <- rbind(
  data.table(
    file_path = list.files(monthly_dir_chirps, "tif$", full.names = T),
    var = "prec"
  ),
  data.table(
    file_path = list.files(monthly_dir_chirts, "tif$", full.names = T),
    var = "tavg"
  )
)
...
files[, year := unlist(tstrsplit(basename(file_path), "-", keep = 2))][,
  month := gsub(
    ".tif",
    "",
    unlist(tstrsplit(basename(file_path), "-", keep = 3))
  )
]
...
combinations <- data.table(
  expand.grid(var = var_choices, m_period = three_month_periods),
  m_period_name = names(three_month_periods)
)

rast_area_class <- terra::rast(lapply(
  1:length(three_month_periods),
  FUN = function(i) {
    ...
    files_hist <- files[
      var == var_choice & year %in% start_years & month %in% m_period
    ][, seq := find_consecutive_pattern(seq = month, pattern = m_period)]
    files_fut <- files[
      var == var_choice & year %in% end_years & month %in% m_period
    ][, seq := find_consecutive_pattern(seq = month, pattern = m_period)]
    ...
    rast_class <- terra::rast(lapply(1:length(thresholds), FUN = function(j) {
      ...
      increase <- classify(
        rast_diff,
        data.frame(
          from = c(-99999, threshold),
          to = c(threshold, 999999),
          becomes = c(0, 1)
        )
      ) *
        rast_area
...
result_area_class <- rbindlist(purrr::map2(
  boundaries_zonal,
  boundaries_index,
  function(zonal_rast, idx) {
    zonal_r <- terra::rast(zonal_rast)
    dat <- zonal(rast_area_class, zonal_r, fun = "sum", na.rm = TRUE)
    dat <- as.data.table(merge(
      dat,
      idx,
      by = "zone_id",
...
rast_area_class_ex <- data.table::melt(
  data.table(result_area_class),
  id.vars = admin
)

rast_area_class_ex <- rast_area_class_ex[,
...
result_area <- rbindlist(purrr::map2(
  boundaries_zonal,
  boundaries_index,
  function(zonal_rast, idx) {
    zonal_r <- terra::rast(zonal_rast)
    dat <- zonal(rast_area, zonal_r, fun = "sum", na.rm = TRUE)
    dat <- as.data.table(merge(
      dat,
      idx,
      by = "zone_id",
...
admin_area_ex <- data.table::melt(data.table(result_area), id.vars = admin)
admin_area_ex[, c("admin2_name", "gaul2_code", "variable") := NULL]
setnames(admin_area_ex, "value", "total_area_km")

rast_area_class_ex <- merge(rast_area_class_ex, admin_area_ex, all.x = T)

rast_area_class_ex[, perc := round(100 * (area_km / total_area_km), 1)][,
  area_km := as.integer(area_km)
][, total_area_km := as.integer(total_area_km)]
