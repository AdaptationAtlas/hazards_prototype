...
packages <- c("terra", "data.table", "arrow", "geoarrow", "countrycode")
...
glps_systems <- terra::classify(
  glps_systems,
  matrix(c(1:15, c(0, 0, 1, 2, 3, 3, 4, 5, 3, 3, 4, 5, 6, 7, NA)), ncol = 2)
)

glps_vals <- data.frame(
  Code = 0:7,
...
econ_loss <- data.table::fread(file.path(
  cattle_heatstress_dir,
  "dmi_and_econ_loss.csv"
))
...
econ_loss <- melt(
  econ_loss,
  id.vars = c("Country", "iso3", "Variable", "Scenario"),
  variable.name = "glps_short"
)
...
econ_loss[, meat_loss := meat_loss * def][, milk_loss := milk_loss * def][,
  def := NULL
]

dmi_econ_loss <- dcast(
  econ_loss,
  glps_short + iso3 ~ Scenario,
  value.var = "dmi"
)
colnames(dmi_econ_loss)[-(1:2)] <- paste0(
  "dmi_",
  colnames(dmi_econ_loss)[-(1:2)]
)
...
colnames(dmi_diff_perc) <- gsub(
  "dmi_",
  "dmi_diff_perc_",
  colnames(dmi_diff_perc)
)
dmi_econ_loss <- cbind(dmi_econ_loss, dmi_diff, dmi_diff_perc)

meat_econ_loss <- dcast(
  econ_loss,
  glps_short + iso3 ~ Scenario,
  value.var = "meat_loss"
)
colnames(meat_econ_loss)[-(1:2)] <- paste0(
  "meat_",
  colnames(meat_econ_loss)[-(1:2)]
)
dmi_econ_loss <- cbind(dmi_econ_loss, meat_econ_loss[, 3:6])

milk_econ_loss <- dcast(
  econ_loss,
  glps_short + iso3 ~ Scenario,
  value.var = "milk_loss"
)
colnames(milk_econ_loss)[-(1:2)] <- paste0(
  "milk_",
  colnames(milk_econ_loss)[-(1:2)]
)
...
glps_systems_masked <- terra::mask(
  terra::crop(glps_systems, countries),
  countries
)

# Rasterize the admin areas
countries_rast <- terra::rasterize(
  countries,
  glps_systems_masked,
  field = "iso3n"
)
...
glps_x_admin_rast <- glps_systems_masked + (countries_rast * 1e4)
...
X[, admin_code := floor(glps / 1e4)]
X[, glps := glps %% 1e4]
X[, iso3 := terra::values(countries)[match(X$admin, countries$iso3n), "iso3"]]
X[,
  admin := terra::values(countries)[
    match(X$admin, countries$iso3n),
    "admin0_name"
  ]
]
...
# Choose admin level
admin_levels <- names(Geographies)[c(1, 2)] # limit to 0 and 1 for notebook
...
heat_impacts <- rbindlist(lapply(1:length(admin_levels), FUN = function(i) {
  ...
  glps_systems_masked <- terra::mask(
    terra::crop(glps_systems, focal_admin),
    focal_admin
  )

  focal_admin$zone_id <- ifelse(
    !is.na(focal_admin$gaul2_code),
    focal_admin$gaul2_code,
    ifelse(
      !is.na(focal_admin$gaul1_code),
      focal_admin$gaul1_code,
      focal_admin$gaul0_code
    )
  )

  # Rasterize the admin areas
  focal_admin_rast <- terra::rasterize(
    focal_admin,
    glps_systems_masked,
    field = "zone_id"
  )
  ...
  # Combine glps number with region code
  glps_x_admin_rast <- glps_systems_masked + (focal_admin_rast * 1e4)
  names(glps_x_admin_rast) <- "glps"
  ...
  X[, admin := floor(glps / 1e4)]
  X[, glps := glps %% 1e4]
  X[,
    iso3 := terra::values(focal_admin)[
      match(X$admin, focal_admin$zone_id),
      "iso3"
    ]
  ]
  X[,
    gaul0_code := terra::values(focal_admin)[
      match(X$admin, focal_admin$zone_id),
      "gaul0_code"
    ]
  ]
  X[,
    gaul1_code := terra::values(focal_admin)[
      match(X$admin, focal_admin$zone_id),
      "gaul1_code"
    ]
  ]
  X[,
    admin0_name := terra::values(focal_admin)[
      match(X$admin, focal_admin$zone_id),
      "admin0_name"
    ]
  ]
  X[,
    admin1_name := terra::values(focal_admin)[
      match(X$admin, focal_admin$zone_id),
      "admin1_name"
    ]
  ]
  ...
  lookup <- terra::values(focal_admin)[
    match(admin_sizes$zone_id, focal_admin$zone_id),
    c("iso3", "gaul0_code", "gaul1_code", "admin0_name", "admin1_name")
  ]
  ...
  X <- merge(
    ...
    admin_sizes,
    all.x = T,
    by = c("iso3", "gaul0_code", "gaul1_code", "admin0_name", "admin1_name")
  )
  ...
  X <- merge(
    X,
    cattle_glps_country[, list(glps_short, iso3, cattle_prod_tot)],
    all.x = T,
    by = c("glps_short", "iso3")
  )
  ...
  # e) Multiply losses by proportion of cattle in that glps x admin combo ####
  target_cols <- grep("meat|milk", colnames(heat_impacts), value = T)
  heat_impacts[,
    (target_cols) := lapply(.SD, function(x) {
      round(1000 * x * cattle_glps_prop)
    }),
    .SDcols = target_cols
  ]
...
filename <- file.path(
  cattle_heatstress_dir,
  "gaul_heat_impact_full_results.parquet"
)
...
heat_impacts_m <- melt(
  heat_impacts[,
    !c("glps_short", "cattle_glps_prop", "glps_area", "admin_unit_ha", "perc")
  ],
  id.vars = c(
    "admin0_name",
    "admin1_name",
...
cattle_glps_country[, admin_tot := sum(cattle_prod_tot), by = iso3][,
  cattle_prod_prop := round(cattle_prod_tot / admin_tot, 3)
]
heat_impacts_m <- merge(
  heat_impacts_m,
  cattle_glps_country[, list(iso3, glps, cattle_prod_prop)],
  all.x = T,
  by = c("iso3", "glps")
)

# Summarize across glps
heat_impacts_dmi <- heat_impacts_m[
  grepl("dmi_diff_perc", variable),
  list(value = weighted.mean(value, cattle_prod_prop, na.rm = T)),
  by = list(iso3, admin0_name, admin1_name, gaul0_code, gaul1_code, variable)
]
heat_impacts_loss <- heat_impacts_m[
  grepl("meat|milk", variable),
...
heat_impacts_m[,
  variable := gsub("dmi_diff_perc", "dry matter intake", variable)
][, timeframe := unlist(tstrsplit(variable, "_", keep = 2))][,
...
heat_impacts_m[
  is.na(admin1_name) & variable %in% c("meat", "milk"),
  list(value = sum(value) / 10^6),
  by = list(timeframe, scenario)
]
heat_impacts_m[
  is.na(admin1_name) & variable %in% c("dry matter intake"),
  hist(value)
]

filename <- file.path(cattle_heatstress_dir, "gaul_heat_impact_simple.parquet")
