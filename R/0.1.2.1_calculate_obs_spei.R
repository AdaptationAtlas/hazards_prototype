...
# R/0.1.2_get_chirps_chirts.R. SPEI is produced at scales 1, 3, 6, 12, 24,
...
#
# Inputs (already on disk after R/0.1.2_get_chirps_chirts.R --full):
...
bootstrap_minimal <- function() {
  ...
  if (!dir.exists(chirts_chirps_hist_dir)) {
    stop("Run R/0.1.2_get_chirps_chirts.R --full before computing SPEI.")
  }
...
if (mode == "--smoke") {
  ...
  cat(
    "Usage:\n",
    "  Rscript R/0.1.2.1_calculate_obs_spei.R --smoke\n",
    "      Single scale (3), Kenya bbox, 1985-2020. Quick correctness check.\n",
    "  Rscript R/0.1.2.1_calculate_obs_spei.R --full\n",
...
for (scale in scales_run) {
  ...
  jsonlite::write_json(list(
    ...
    parent_script = "R/0.1.2.1_calculate_obs_spei.R",
