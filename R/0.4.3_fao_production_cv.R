...
# R/0.4.2_create_crop_vop_nominal_usd.R
...
# Create functions ####
transform_fao <- function(data, element, exclude_units, lps2fao, spam2fao, remove_countries, keep_years, add_missing = TRUE) {
...
fill_gaps <- function(data, neighbors, regions) {
  ...
  avg_neighbors <- function(data, crop_focus, iso3_focus, neighbors) {
    ...
    result <- data[iso3 %in% neighbors_focal & crop == crop_focus, mean(value, na.rm = TRUE)]
    return(result)
  }
  ...
  avg_regions <- function(data, iso3_focus, crop_focus, regions) {
    region_focal <- names(regions)[sapply(regions, FUN = function(X) {
      iso3_focus %in% X
    })]
    neighbors_focal <- regions[[region_focal]]
    result <- data[crop == crop_focus & iso3 %in% neighbors_focal, mean(value, na.rm = TRUE)]
    return(result)
  }
  ...
  data[, mean_continent := mean(value, na.rm = TRUE), by = crop]

  return(data)
}
calc_cv <- function(data, detrend = TRUE, rm.na = TRUE, min_data = 10, prob_req = 0.05, rsq_req = 0.1) {
  if (sum(is.na(data)) > 0 & rm.na == FALSE) {
...
if (FALSE) {
  ...
  for (i in 1:length(X)) {
    print(i)
    values <- X[[i]]$value
    calc_cv(data = values, detrend = TRUE)
    calc_cv(data = values, detrend = FALSE)
  }
}
# Apply CV function ####
min_data <- 10
...
cv <- data[, list(
  cv = calc_cv(data = value, detrend = FALSE, min_data = min_data, prob_req = prob_req, rsq_req = rsq_req),
  cv_detrended = calc_cv(data = value, detrend = TRUE, min_data = min_data, prob_req = prob_req, rsq_req = rsq_req),
...
if (FALSE) {
  cv[, cv_prod_diff := cv_prod - cv_detrended_prod][, cv_yield_diff := cv_yield - cv_detrended_yield][, cv_diff := cv_prod - cv_yield][, cv_detrended_diff := cv_detrended_prod - cv_detrended_yield]
...
data[, value_filled := value][is.na(value_filled), value_filled := mean_neighbors][is.na(value_filled), value_filled := mean_region][is.na(value_filled), value_filled := mean_continent]

data <- merge(data, unique(data.frame(geoboundaries)[, c("iso3", "admin0_name")]), by = "iso3", all.x = TRUE)
