...
> - **⚠️ CORRECTION (audit 2026-06-27): do NOT use `FORCE_OVERWRITE=1`** — there is NO timeframe env filter in R/2 or R/3, so FORCE rebuilds ALL timeframes incl. still-inflated FUTURE NDWS, and `s3_upload` (no skip logic) re-ships future keys. **Historic-only scoping = FORCE UNSET + pre-delete only the historic-NDWS artifacts** so `file.exists` skips intact future/non-NDWS. Enable §3/§5.3 via the toggles (independent of FORCE).
...
> - **R/1**: delete historic NDWS in `indices_dir2/{annual,jagermeyr}` → `Rscript R/1_make_timeseries.R` (overwrite=FALSE → rebuilds only deleted).
> - **R/2**: FORCE UNSET; pre-delete historic-NDWS in haz_time_class/risk/int + **every NDWS-bearing `_int` in hazard_risk** (NDWS reaches R/3 only via `_int` compound files — R/3:1175,1188 keep type=="int" — so delete ALL `_int` whose combo contains NDWS, not just standalone). Then `RUN_R2_RUN3=1 RUN_R2_RUN5_3=1 RUN_R2_RUN5_2=1 Rscript R/2_calculate_haz_freq.R`. Run `Rscript R/probe_r2_5_2_vec.R` first.
> - **R/3**: delete stale NDWS `_int` TIFs+parquets in hazard_risk_vop[_usd] → `Rscript R/3_freq_x_exposure.R` (FORCE UNSET). §4.1+§4.2, vop+vop_usd, both axes.
> - **R/3.1 — SKIP (confirmed no-op)**: `3.1_season_weightings.R` computes value_adj (L212-224) but NEVER writes it (no write in the loop or the legacy section L234-295) → produces no persisted product. Skip for the rebake. (Flag to Pete separately if season-weighted hazard_exposure was meant to be live — it's currently WIP/non-writing.)
> - **Preconditions** (NEXT_FULL_REBAKE.md): exposure vintage #10 (keep existing MapSPAM-2020/GLW4-2020 for an NDWS-only fix unless new vintage intended); CR-115 `aggregate_disputedRegions` is UNWIRED (zero call sites) → dup adm0 rows persist by design, no action. Pattern-B #12 denominator is external → not closed by this. Riding items (#13 poultry, CR-093) carry along, fine.
