...
calc_ndwl0 <- function(yr, mn){
  ...
  if (!should_skip(outfile)) {   # honours FORCE_OVERWRITE=1
    ...
    if(date %in% c('1995-01','2021-01')){
      ...
      # month now fails LOUD instead of silently seeding wrong.
      pdate <- as.Date(paste0(yr,'-',mn,'-01')) - 1
      prior_avail <- paste0(dirname(outfile),'/AVAIL-',format(pdate,'%Y'),'-',format(pdate,'%m'),'.tif')
      stopifnot("prior-month AVAIL missing - run months in order from the seed month" = file.exists(prior_avail))
    ...
    NDWL0  <- sum(LOGGING > 0)
    terra::writeRaster(NDWL0, outfile, overwrite = TRUE)
