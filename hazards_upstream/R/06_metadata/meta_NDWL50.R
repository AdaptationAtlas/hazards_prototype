...
if (!skip_preparation) {
  ...
  1:nrow(stp) %>%
    purrr::map(.f=function(i) {
      continuous_map(index="NDWL50", HS.stat=NULL, period=stp$prd[i], scenario=stp$sce[i], domean=TRUE, domedian=TRUE, domax=TRUE, doensemble=TRUE, omitcalendar=FALSE)
    })
}
...
if (!skip_preparation) {
  ...
  1:nrow(stp) %>%
    purrr::map(.f=function(i) {
      category_map(index="NDWL50", HS.stat=NULL, period=stp$prd[i], scenario=stp$sce[i], gcm=stp$gcm[i], stat=stp$stat[i])
    })
}
