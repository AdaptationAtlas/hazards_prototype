#Create Atlas metadata for TAVG: monthly mean temperature
...
if (!skip_preparation) {
  ...
  1:nrow(stp) %>%
    purrr::map(.f=function(i) {
      continuous_map(index="TAVG", HS.stat=NULL, period=stp$prd[i], scenario=stp$sce[i], domean=TRUE, domedian=TRUE, domax=TRUE, doensemble=TRUE, omitcalendar=FALSE)
    })
}
...
if (!skip_preparation) {
  ...
  1:nrow(stp) %>%
    purrr::map(.f=function(i) {
      category_map(index="TAVG", HS.stat=NULL, period=stp$prd[i], scenario=stp$sce[i], gcm=stp$gcm[i], stat=stp$stat[i])
    })
}
...
#directories
his_dir <- paste0(wd, "/indices/historical/TAVG")
ssp_dir <- paste0(wd, "/indices/", full_tb$fullname, "/TAVG")
...
fnames <- gsub(pattern="/TAVG/long_term_stats", replacement="", x=fnames)
fnames <- gsub(pattern="/TAVG", replacement="", x=fnames)
names(r_data) <- fnames

#write metadata
tavg_meta <- atlas_metadata(data = r_data,
                            folder = meta_dir,
                            dataset.title_short = "TAVG",
                            dataset.title_long = "Average temperature per month (Celsius)",
                            dataset.desc = "Average of daily temperature per month and per year for historical and future periods, calculated using CHIRTS daily temperature data.",
                            ...
                            dataset.citation = "Ramirez-Villegas, J., Achicanoy, H. 2023. CMIP6 climate hazards: average monthly temperature. CGIAR. Dataset.",
                            dataset.licence = "CC-BY-4.0",
                            file.filename = "TAVG.tif",
                            file.format = "GeoTiff",
                            ...
                            variable.subname = "average temperature",
                            ...
                            variable.unit = "Celsius",
                            method.analysis_type = "average of daily temperature",
                            method.description = "Average daily temperature per month.",
...
out_file <- file(paste0(meta_dir,"/TAVG.txt"), open="w")
for (i in 1:ncol(tavg_meta)) {
  writeLines(text=names(tavg_meta)[i], con=out_file)
  writeLines(text=paste0(tavg_meta[1,i], "\n"), con=out_file)
}
...
terra::writeRaster(r_data, paste0(buck_dir, "/TAVG.tiff"), overwrite=TRUE)
