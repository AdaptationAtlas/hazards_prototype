...
packages <- c("terra","shinydashboard","data.table","shiny","stringi","arrow","ggplot2","DT") # Ensure correct package names
...
files_class_filtered <- reactive({
  ...
  files_int_filtered<-reactive({
    files_int<-list.files(paste0(haz_time_int_dir(),"/",int_name()),full.names = T)
  ...
  #X) VoP files ####
   files_vop_filtered <- reactive({
     int_name2<-unlist(strsplit(int_name(),"[+]"))
     int_name2<-gsub("PTOT-L","PTOT_L-",int_name2)
     int_name2<-gsub("PTOT-G","PTOT_G-",int_name2)
     int_name2<-gsub("TAVG-G","TAVG_G-",int_name2)
     int_name2<-unlist(tstrsplit(int_name2,"-",keep=1))
     int_name2<-paste0(int_name2,collapse="+")
      ...
      files_vop()[grepl("admin0",files_vop()) & 
                  ...
                  grepl(int_name2,files_vop(),fixed=T)]
  })
  ...
  output$plot_risk_int1<-renderPlot({
    plot(data()[[1:4]],nc=4,pax=list(cex.axis = 2), cex.main = 2,mar= c(4.1, 4.1, 2.1, 2.1),plg=list(cex=2))
  }) 
  output$plot_risk_int2<-renderPlot({
    plot(data()[[5:8]],nc=4,pax=list(cex.axis = 2), cex.main = 2,mar= c(4.1, 4.1, 2.1, 2.1),plg=list(cex=2))
  }) 
  ...
  vop_data<-reactive({
    ...
    data<-data[hazard!="any"
                   ...
                   ][,list(value=sum(value,na.rm=T)),by=list(crop,hazard)
                     ][,total_value:=sum(value,na.rm=T),by=crop
  ...
  ggplot(vop_data_ordered, aes(x = crop, y = plot_value, fill = hazard)) +
