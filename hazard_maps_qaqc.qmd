...
packages <- c("terra","shinydashboard","data.table","shiny","stringi") # Ensure correct package names
...
files_timeseries<-list.files(haz_timeseries_dir,".tif$",full.names = T)
files_class<-list.files(haz_time_class_dir,".tif$",full.names = T)
files_risk<-list.files(haz_time_risk_dir,".tif$",full.names = T)
...
combinations<-rbind(combinations_c,combinations_a)[crop!="generic"][,heat1:=unlist(tstrsplit(heat,"-",keep=1))
                                                                                    ...
                                                                                    ][,combination:=paste0("heat:",heat1," wet:",wet1," d:",dry1)]
...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    sidebarMenu(
      menuItem("Data Filtering", tabName = "data_filtering", icon = icon("filter")),
            selectInput("scenario", "Choose scenario:", choices = unique(Scenarios$combined), selected = "historic"),
            selectInput("hazard", "Choose hazard combination:", choices = unique(combinations$combination), selected = "PTOT_L"),
            selectInput("crop", "Choose crop:", choices = unique(haz_class$crop), selected = "maize"),
  ...
  dashboardBody(
        fluidRow(
          ...
          box(plotOutput("plot_timeseries"),title="Timeseries (year1)"),
          box(plotOutput("plot_class"),title="Classified timeseries (year1)"),
          box(plotOutput("plot_risk"),title="Hazard risk (over timeseries)"),
          box(plotOutput("plot_risk_int"),title = "Hazard risk interactions")
        )
  )
  )
```
...
#0) Create hazards ####
  combinations_filtered<-reactive({
    combinations[combination == input$hazard & severity_class==input$severity & crop==input$crop,]
    })
...
#1) Hazard classes  ####
   ...
   files_timeseries_filtered <- reactive({
    c(
      heat = files_timeseries[grepl(unlist(tstrsplit(input$scenario,"-",keep=1)),files_timeseries) & 
                       ...
                       grepl(unlist(gsub("_L|_H","",hazards_filtered()[1])),files_timeseries)],
      wet = files_timeseries[grepl(unlist(tstrsplit(input$scenario,"-",keep=1)),files_timeseries) & 
                       grepl(unlist(tstrsplit(input$scenario,"-",keep=2)),files_timeseries) & 
                       grepl(unlist(gsub("_L|_H","",hazards_filtered()[2])),files_timeseries)],
      dry = files_timeseries[grepl(unlist(tstrsplit(input$scenario,"-",keep=1)),files_timeseries) & 
                       grepl(unlist(tstrsplit(input$scenario,"-",keep=2)),files_timeseries) & 
                       grepl(unlist(gsub("_L|_H","",hazards_filtered()[3])),files_timeseries)]
    )
    })
  ...
  files_risk_filtered <- reactive({
     c(
      heat = files_risk[grepl(unlist(tstrsplit(input$scenario,"-",keep=1)),files_class) & 
                  grepl(unlist(tstrsplit(input$scenario,"-",keep=2)),files_class) &
                  grepl(unlist(gsub("_L|_H","",hazards_filtered()[1])),files_class) &
                  grepl(thresholds_filtered()[1],files_class)],
      wet = files_risk[grepl(unlist(tstrsplit(input$scenario,"-",keep=1)),files_class) & 
                  grepl(unlist(tstrsplit(input$scenario,"-",keep=2)),files_class) &
                  grepl(unlist(gsub("_L|_H","",hazards_filtered()[2])),files_class) &
                  grepl(thresholds_filtered()[2],files_class)],
      dry = files_risk[grepl(unlist(tstrsplit(input$scenario,"-",keep=1)),files_class) & 
                  grepl(unlist(tstrsplit(input$scenario,"-",keep=2)),files_class) &
                  grepl(unlist(gsub("_L|_H","",hazards_filtered()[3])),files_class) &
                  grepl(thresholds_filtered()[3],files_class)]
     )
  ...
  files_int_filtered<-reactive({
    files_int<-list.files(paste0(haz_time_int_dir,"/",combinations_filtered()[,paste0(c(dry,heat,wet),collapse="+")]),full.names = T)
  ...
  output$plot_risk<-renderPlot({
    ...
    plot(data)
      ...
      # 9)  Hazard risk interaction ####
  output$plot_risk_int<-renderPlot({
